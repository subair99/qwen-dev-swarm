...
- **True Sandbox Isolation**: I learned that `docker run` alone is insufficient for security. True isolation requires a combination of flags: `--network none`, `--read-only`, `--user 1000:1000`, `--pids-limit`, and cgroup-enforced memory/CPU caps. The attack surface $\mathcal{A}$ approaches zero when:
  $$
  \mathcal{A} \approx \varnothing \quad \text{under} \quad (\text{net}=\varnothing) \land (\text{fs}=\text{ro}) \land (\text{uid} \neq 0)
  $$
- **Streaming LLM Responses & Buffer Management**: Parsing `<thinking>` tags and code blocks in real-time requires a non-blocking state machine. A naive buffer causes deadlocks when LLMs output `<` in code. I implemented a safe-length buffer where $B_{\text{safe}} = \max(|\text{tag}|) - 1$, ensuring zero character latency while preserving tag boundaries.
...
3. **Multi-Layer Prompt Defense (`swarm/guardrails.py`)**  
   ...
   $$
   \mathcal{G}(x) = \text{True} \iff \bigwedge_{i=1}^{3} L_i(x) = \text{PASS}
   $$

4. **Orchestrator & Self-Correction Loop (`orchestrator.py`)**  
   Engineered the state machine that manages generation, sandbox execution, QA analysis, and retries. Implemented bounded retry logic where $n \leq N_{\text{max}}$ with decaying hint injection to prevent oscillation. Added mandatory HITL checkpoints before any sandbox execution.
...
7. **Hardening & Polish**  
   Integrated `uv` for cryptographic dependency locking, enforced `.env` permission checks (`chmod 600`), added mandatory module docstring validation via regex, and implemented path traversal sanitization: $B_\text{sanitize\_path}(p) \rightarrow \text{basename}(p)$.
...
|:---|:---|:---|
| **Infinite Retry Loops** | QA feedback sometimes caused the Coder to oscillate between two flawed implementations. | Implemented bounded retries $n \leq N_{\text{max}}$ with a convergence threshold. If $\Delta \text{error}_t < \epsilon$ for 3 iterations, the loop forces a structural rewrite. |
| **Sandbox Resource Leaks** | Early Docker runs allowed unlimited PIDs and memory, enabling fork bombs. | Enforced cgroup limits: `--memory=512m --cpus=1.0 --pids-limit=50`. Added `--tmpfs /tmp:noexec` to block binary drops. |
| **Streaming Parser Deadlocks** | LLMs output `<` in comparisons (`if a < b:`), causing the `<thinking>` buffer to hang. | Built a non-blocking state machine with safe-length buffering: only retain $|\text{tag}|-1$ characters when searching for delimiters. |
| **Flaky AI-Generated Tests** | Tests used `time.time()` or `assertLess(elapsed, threshold)`, causing CI failures. | Added explicit guardrails in `Software_Architect` and `Code_Reviewer` prompts forbidding time-based assertions. Enforced value-correctness tests: $\text{assert } f(x) = y_{\text{known}}$. |
| **Prompt Injection Evasion** | Attackers used Unicode escapes, zero-width chars, and semantic role-play to bypass regex. | Implemented 3-tier defense. Semantic layer uses a fast model to compute intent confidence: $P(\text{malicious} \mid x) > \theta \Rightarrow \text{BLOCK}$. |
...
| **Missing Module Docstrings** | LLMs often skipped the mandatory top-level `"""..."""` block. | Added post-processing regex check: `re.match(r'^\s*"""', code)`. If false, triggers a one-shot rewrite prompt that prepends documentation without altering logic. |
